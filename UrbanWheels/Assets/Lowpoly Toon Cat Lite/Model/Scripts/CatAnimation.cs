...
{
/*
  Fainting was W, now it's F.
  stand up from faint: E
  take damage but stay up: Q
  Run: Z + arrow key?
  Jump high: S
  */
...
[SerializeField]
  ...
  // private static adonly int FaintTag = Animator.StringToHash("Faint");
  ...
  void Start()
  {
    ...
    // }
    // catch{
    //   Debug.Log("Unable to instantiate _MeshRenderer");
    // }
  }
  ...
  private void MOVE ()
  {
    ...
    if(_Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == Animator.StringToHash("Base Layer.Walk") || _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == Animator.StringToHash("Base Layer.Walk 0") || _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == Animator.StringToHash("Base Layer.Walk 1"))
      {
        ...
        _Animator.SetFloat(SpeedParameter, speed);
        // if(this.transform.position.x < -7.9 || this.transform.position.x > 10){
        //     _Animator.Play("Idle_A");
        // }
        // if(this.transform.position.x < -9.5 || this.transform.position.x > 8.4){
        //     _Animator.Play("Idle_A");    
        //     this.transform.Rotate(Vector3.up, 180.0f);
        //     _Animator.Play("Idle_A");
        //     _Animator.SetFloat(SpeedParameter, 1);
        //     _Animator.Play("Walk");
        // }
      }

    if(_Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == Animator.StringToHash("Base Layer.Run 1") || _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == Animator.StringToHash("Base Layer.Run 0") || _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == Animator.StringToHash("Base Layer.Run"))
      {
        speed = 4;
        _Animator.SetFloat(SpeedParameter, speed);
        Vector3 velocity = this.transform.rotation * new Vector3(0, 0, speed);
...
//   private void FAINT ()
//   {
    //If all commented out, nothing happens

    // if (!_Status[Faint]){
    // if (Input.GetKeyDown(KeyCode.F))
    // {
      //If only this is uncommented, then character puts up his arms
    ...
    //     && !_Animator.IsInTransition(0))
    // {
      //If only this is uncommented, then character falls down
...
// void OnCollisionEnter(Collision c)
    ...
    //   isCollided = true;
        ...
        // {
            ...
            //EventManager.TriggerEvent<ObstacleCollisionEvent, Vector3>(c.contacts[0].point);
